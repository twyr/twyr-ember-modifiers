...
export default class ElementResizeWatcherService extends Service {
	...
	constructor() {
		...
		this.#observer = new ResizeObserver(this.#notifyResize?.bind?.(this));
	}
	// #endregion
	...
	#notifyResize(entries) {
		this.#debug?.(`#notifyResize:`, entries);
		entries?.forEach?.((entry) => {
			this.#debug?.(
				`#notifyResize::processing resize entry:`,
			...
			this.#debug?.(`#notifyResize::target element:`, element);
			const callbacks = this.#elementCallback?.get?.(element);
			if (!callbacks) {
				this.#debug?.(
					`#notifyResize::target element callback not found. aborting...`
				);
				return;
			}
